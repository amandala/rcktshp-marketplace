...
function hrb_agreement_cancel( $proposal, $user ) {
	...
	wp_update_post( array( 'ID' => $proposal->get_post_ID(), 'post_status' => HRB_PROJECT_STATUS_CANCELED_TERMS ) );
...
function hrb_proposal_selected( $proposal ) {
	...
	wp_update_post( array( 'ID' => $project_id, 'post_status' => HRB_PROJECT_STATUS_TERMS ) );
