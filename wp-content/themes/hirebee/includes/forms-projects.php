...
class HRB_Project_Form_Edit extends APP_Checkout_Step {
	...
	function validate_fields( $order, $checkout ){
        ...
        if ( ! hrb_charge_listings() && ( empty( $_POST['duration'] ) && $hrb_options->project_duration > 0 ) ) {
...
class HRB_Project_Form_Relist_Free extends HRB_Project_Form_Relist {
	...
	function display( $order, $checkout ) {
        ...
        $template_vars = apply_filters( 'hrb_project_relist_free_template_vars', $template_vars );
