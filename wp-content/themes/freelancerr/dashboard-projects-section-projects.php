<div id="projects">
								...
								<div class="large-10 columns projects-section">
									<div class="row project-title-row">
										<div class="large-12 small-12 columns">
											<h2><?php the_hrb_child_project_title(); ?></h2>
										</div>
										<div class="large-8 small-8 columns project-meta-info">
									...
									<div class="row section-secondary-info">
										<div class="large-7 small-2 columns">
											<span data-tooltip title="<?php echo esc_attr( __( 'Project Status', APP_TD ) ); ?>" class="label project-status <?php echo esc_attr( get_the_hrb_project_or_workspace_status() ); ?>"><span class="project-status-label">Project Status:</span> <i class="icon i-status"></i> <b><?php the_hrb_project_or_workspace_status(); ?></b></span>
										</div>
										...
										<div class="large-3 small-4 columns projects-actions">
											<?php rcktshp_the_hrb_dashboard_project_actions( $post ); ?>
													...
													<?php foreach( $participants->results as $worker ): ?>
															...
															<div class="row project-participants-meta">
																<div class="large-5 columns work-status <?php echo esc_attr( $worker->status ); ?> <?php echo get_the_hrb_project_or_workspace_status(); ?>">
																	<span data-tooltip class="label <?php echo esc_attr( $worker->status ); ?>" title="<?php echo esc_attr( __( 'Work Status', APP_TD ) ); ?>"><i class="icon i-work-status"></i></i> <b><?php echo hrb_get_participants_statuses_verbiages( $worker->status ); ?></b></span>
																		<?php if ( $dispute && 'publish' == $dispute->post_status ) : ?>
																				<div class="label dispute-status "><i class="icon i-dispute"></i><?php echo __( 'Dispute Opened', APP_TD ); ?></div>
																		<?php endif; ?>
																</div>
																...
																<div class="large-3 small-4 columns workspace-actions">
																	<?php rcktshp_the_hrb_dashboard_project_work_actions( $post, $proposal ); ?>
		...
		<?php else: ?>
			...
			<?php if ( current_user_can( 'edit_projects') ): ?>
					...
					<h5 class="no-results"><?php echo "No projects found" ?>
					<div id="no-proj-buttons">
					<?php
						$post_url = network_site_url('post-a-project/');
						$find_url = network_site_url('projects/');
						echo "<a  id ='no-proj-button-pad' class='button small orange' href='" . $post_url . " ' >Post a project</a>";
						echo "<a  class='button small teal' href='" . $find_url . " ' >Find Work</a>";
					?>
					</div>
					</h5>
			...
			<?php else: ?>
					...
					<?php
						$post_url = network_site_url('post-a-project/');
						$find_url = network_site_url('projects/');
						echo "<a  class='button small orange' href='" . $post_url . " ' >Post a project</a>";
						echo "<a  class='button small teal' href='" . $find_url . " ' >Find Work</a>";
					?>
					</div>
					</h5>
